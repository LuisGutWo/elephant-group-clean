...
// Local/dev fallback:
// Next.js should load .env.local automatically, but some local setups fail.
...
export default async function handler(req, res) {
  ...
  // In local development we skip CSRF validation because some browsers/clients
  // may not send cookies reliably to Next.js API routes.
  // This prevents "Failed to fetch" / 403 during initial local deploy testing.
  const isDevLocal = process.env.NODE_ENV === "development";
  if (!isDevLocal && !isValidCsrf(req)) {
